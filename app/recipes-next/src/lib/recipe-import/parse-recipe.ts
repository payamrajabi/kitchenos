...
6. INGREDIENT NAME: Use simple, singular, AP-style Title Case names in the "ingredient" field (e.g. "Garlic" not "3 cloves of fresh garlic", "Tomato Paste" not "tomato paste"). Strip brand names, marketing language, and "organic/fresh" qualifiers unless they change the ingredient identity (e.g. keep "Fresh Mozzarella" vs "Mozzarella"). CUT FORMS ARE NOT IDENTITY: shapes like "wedges", "slices", "rounds", "halves", "quarters", "dice", "cubes", "strips", "sticks" are preparations applied to a base ingredient, not different ingredients. Emit the base ingredient in "ingredient" and move the cut form into "preparation". For citrus specifically, always use "Lemon" or "Lime" as the ingredient — never "Lemon Wedges" or "Lime Wedges" — with "wedges" in "preparation".
7. INGREDIENT PREPARATION: Put preparation/state phrases into "preparation" — not into "ingredient". Examples: "finely chopped", "divided", "at room temperature", "cut into 1-inch cubes", "wedges", "thinly sliced", "cut into rounds", "quartered", "to serve", "for garnish". null if no prep state is implied.
...
10. INSTRUCTION TEXT — PRESERVE VERBATIM: The "text" field for each step MUST use the source's exact wording. Do NOT summarise, paraphrase, condense, or rewrite. Every quantity, temperature, time, piece of equipment, and technique cue must appear in "text" exactly as the source stated it (e.g. if the source says "Peel and par-cook the 1.5 kg russet potatoes and place them in a pot of cold water. Bring to a gentle boil and cook for 6 to 8 minutes.", the "text" field contains that full sentence, not a shortened version). The ONLY permitted edits are: fixing obvious typos, normalising whitespace/quotes, and splitting a single source paragraph into multiple steps when it clearly describes multiple discrete actions (in which case each step's text is still the verbatim sentence(s) from the source that describe that action). You MAY omit purely editorial filler that is unambiguously NOT cooking instruction (e.g. "I love this part!", "my grandma always said…") — but when in doubt, keep it. It is better to carry a little extra prose than to drop a time, temperature, or quantity. Never invent wording that is not in the source.
11. INSTRUCTION HEADING — GENERATED SUMMARY: Every step MUST include a "heading" — a short, action-led summary of what that step accomplishes, generated by you. It is the label a cook uses to scan the method at a glance, not the full instruction. This is the ONLY place where you summarise; the "text" stays verbatim (see rule 10). Requirements:
